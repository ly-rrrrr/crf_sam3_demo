"""
Run CRF-SAM3 sub-expression/expr scoring over a dataset using the standalone pipeline.

Usage:
    python run_crf_sam3_dataset_subexpr.py --config Visual-RFT/configs/crf_sam3_config_subexpr.yaml
"""
...
# make Visual-RFT importable
ROOT = os.path.dirname(os.path.abspath(__file__))
VISUAL_RFT_DIR = os.path.join(ROOT, "Visual-RFT")
if VISUAL_RFT_DIR not in sys.path:
    sys.path.append(VISUAL_RFT_DIR)

from crf_sam3_demo_subexpr_full import CRFSAM3PipelineSubexpr
...
def identify_gt_objects(selected_masks, gt_masks_dict, iou_threshold=0.1):
    """
    Identify GT objects among selected masks by IoU with ground truth.
    """
    if len(selected_masks) == 0 or len(gt_masks_dict) == 0:
        return [], {}

    all_obj_ids = set()
    for frame_masks in selected_masks.values():
    ...
    for obj_id in all_obj_ids:
        obj_masks = {}
        for frame_idx, frame_masks in selected_masks.items():
            if obj_id in frame_masks:
                obj_masks[frame_idx] = frame_masks[obj_id]
        if len(obj_masks) > 0:
...
def main():
    parser = argparse.ArgumentParser(
        description="Run CRF-SAM3 on MeViS dataset (sub-expression prompt)",
...
Examples:
  python run_crf_sam3_dataset_subexpr.py --config Visual-RFT/configs/crf_sam3_config_subexpr.yaml
    ...
    logger.info("="*60)
    logger.info("CRF-SAM3: Sub-expression/Expr Discrimination (standalone pipeline)")
    logger.info("="*60)
    ...
    prompt_type = config.get('discrimination', {}).get('prompt_type', 'subexpr')
    if prompt_type == "expr":
        prompt_template_path = "Visual-RFT/configs/discrimination_prompt_expr_mask.txt"
    else:
        prompt_template_path = "Visual-RFT/configs/discrimination_prompt_subexpr_mask.txt"

    visualization_style = config.get('visual_tube', {}).get('visualization_style', 'mask')
    sampling_strategy = config.get('visual_tube', {}).get('sampling_strategy', 'uniform')
    custom_frame_indices = config.get('visual_tube', {}).get('custom_frame_indices', None)
    concept_use_qframe = config.get('concept_extraction', {}).get('use_qframe', False)

    # Q-Frame configuration (only needed if sampling_strategy == "qframe" or concept_use_qframe)
    qframe_config = None
    ...
    pipeline = CRFSAM3PipelineSubexpr(
        mllm_path=mllm_path,
